...
bool Manager::keydown(Undo& undo, SDL_Keysym key, Scene& scene, Camera& cam) {
    ...
    if(key.mod & mod) {
        ...
        case SDLK_s: save_scene(scene, undo); return true;
        }
    }
    ...
    case Mode::simulate: return simulate.keydown(widgets, undo, key);
    }
    return false;
}
...
void Manager::load_scene(Scene& scene, Undo& undo, bool clear) {
    ...
    after_save = [this, &scene, &undo, clear](bool success) {
        ...
        std::string error = scene.load(load_opt, undo, *this, std::string(path));
        set_error(error);

        if(clear && error.empty()) {
            n_actions_at_last_save = undo.n_actions();
...
void Manager::particles_edit_gui(Undo& undo, Scene_Particles& particles) {

    static Scene_Particles::Options old_opt;
    Scene_Particles::Options start_opt = particles.opt;
    Scene_Particles::Options& opt = particles.opt;
    ...
    ImGui::ColorEdit3("Color", opt.color.data);
    activate();
    ImGui::DragFloat("Speed", &opt.velocity, 0.1f, 0.0f, std::numeric_limits<float>::max(), "%.2f");
    activate();
    ...
    if(U) {
        undo.update_particles(particles.id(), old_opt);
    }
}
...
Mode Manager::item_options(Undo& undo, Mode cur_mode, Scene_Item& item, Pose& old_pose) {
    ...
    if(item.is<Scene_Object>()) {
        ...
        if(ImGui::CollapsingHeader("Edit Emitter")) {
            ImGui::Indent();
            particles_edit_gui(undo, particles);
            ImGui::Unindent();
        }
    }
    return cur_mode;
...
void Manager::light_edit_gui(Undo& undo, Scene_Light& light) {
    ...
    case Light_Type::spot: {
        ...
        if(light.opt.angle_bounds.x > light.opt.angle_bounds.y) {
            light.opt.angle_bounds.x = light.opt.angle_bounds.y;
        }
        activate();
    } break;
    default: break;
...
void Manager::UIsidebar(Scene& scene, Undo& undo, float menu_height, Camera& cam) {
    ...
    case Mode::simulate: {
        mode = simulate.UIsidebar(*this, scene, undo, widgets, selected);
    } break;
...
void Manager::UIsettings() {
    ...
    ImGui::Checkbox("Triangulate", &load_opt.triangulate);
    ImGui::Checkbox("Generate Normals", &load_opt.gen_normals);
    ImGui::Checkbox("Generate Smooth Normals", &load_opt.gen_smooth_normals);
    ImGui::Checkbox("Fix Infacing Normals", &load_opt.fix_infacing_normals);
    ImGui::Checkbox("Debone", &load_opt.debone);
...
void Manager::clear_select() {
    ...
    case Mode::animate:
    case Mode::layout:
    case Mode::simulate: layout.clear_select(); break;
...
void Manager::refresh_anim(Scene& scene, Undo& undo) {
    animate.refresh(scene);
    simulate.update_bvh(scene, undo);
}
...
void Manager::render_3d(Scene& scene, Undo& undo, Camera& camera) {
    ...
    case Mode::simulate: {
        simulate.render(selected, widgets, camera);
    } break;
...
void Manager::hover(Vec2 pixel, Vec3 cam, Vec2 spos, Vec3 dir) {
    if(mode == Mode::model) {
        model.hover(Renderer::get().read_id(pixel));
    } else if(mode == Mode::rig) {
